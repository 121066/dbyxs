<template >
  <div class="n_comer_favor_see-container">
    ...
    <div class="content">
      <div class="query">
        ...
        <div class="margin_right_20">
          参与者昵称:&nbsp;
          <el-input v-model="value5" placeholder="请输入内容" style="width:100px;height:30px;"></el-input>
        </div>
        <div class="margin_right_20">
          拼团状态:&nbsp;
        ...
        <el-table :data="activityList" style="width: 100%" height="598">
          ...
          <el-table-column label="拼团状态">
            <template slot-scope="scope">
              <span>{{options1[scope.row.status].label}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="endTime" label="创建结束时间" width="200"></el-table-column>
...
import {nc_favor_teams} from '../../api/getData'
export default {
  ...
  methods: {
    ...
    jump_detail(id,status) {
      ...
      localStorage.setItem('path',JSON.stringify(this.$route.path))
      if (status == "1") {
        this.$router.push("nComer_favor_success_detail?id=" + id);
      } else if (status == "3") {
        this.$router.push("nComer_favor_await_detail?id=" + id);
      } else {
        this.$router.push("nComer_favor_await_detail?id=" + id);
      }
    ...
    get_list(currentPage) {
      ...
      let data =
        ...
        "&current=" +
        this.currentPage;
      nc_favor_teams(data).then(result => {
        if (result.data.status == 0) {
          ...
          if (result.data.data.records == "") {
            ...
            data.forEach(item => {
              ...
              if(item.winner){
                var winer = item.winner;
              } else {
                var winer = '------'
              }
              var createTime = item.createtime;
              if(item.overTime){
                var endTime = item.overTime;
              } else {
                var endTime = '------';
              }
