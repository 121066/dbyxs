  ...
  <div class="homeSetUp-container">
    ...
    <div class="content">
      <div class="query">
        ...
        <div class="btn">
          <el-button type="primary" icon="el-icon-search" @click="get_puchase_list(1)">查询</el-button>
        </div>
      </div>
      <div class="table">
        <el-table :data="purList" style="width: 100%" height="598">
          <el-table-column prop="username" label="发起者"></el-table-column>
          <el-table-column prop="shortName" label="商品名称"></el-table-column>
          <el-table-column prop="goodsType" label="所属类型">
          </el-table-column>
          <el-table-column prop="price" label="价格(元)">
          </el-table-column>
          <el-table-column prop="gpPice" label="拼单价格(元)">
          </el-table-column>
          <el-table-column  label="状态">
            <template slot-scope="scope">
              <span>{{statusList[parseInt(scope.row.status)]['label']}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="launchNum" label="发起人数(人)">
          </el-table-column>
          <el-table-column prop="joinNum" label="参与人数(人)">
          </el-table-column>
          <el-table-column prop="startTime" label="开始时间" width="170px"></el-table-column>
          <el-table-column prop="endTime" label="结束时间" width="170px"></el-table-column>
...
import { purchase_list } from "../../api/getData";
...
export default {
  ...
  methods: {
    ...
    jump_detail(id) {
      ...
      this.$router.push("/sin_detail?id=" + id);
    },
    get_puchase_list(current){
      if(current){
        this.currentPage = current
      }
      this.purList = []
      var data = 'current=' + this.currentPage + '&size=' + this.pageSize + '&goodsName=' + 
        this.goodsName + '&goodsTypeId=' + this.type + '&username=' + this.initiator + '&status=' + this.status +
        '&startTime' + this.startTime + '&endTime=' + this.endTime
      purchase_list(data).then(res => {
      if(res.data.status == 0 && res.data.data.records.length != 0){
        this.total = res.data.data.total
        this.page = Math.ceil(this.total / this.pageSize)
        this.purList = res.data.data.records
      }
      })
    },
