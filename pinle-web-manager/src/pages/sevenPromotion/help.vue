<template>
  <div class="sevenhelp-container">
    ...
    <div class="content">
      ...
      <div class="table">
        <el-table :data="usersList" style="width: 100%" height="528" 
...
export default {
  ...
  methods: {
    ...
    get_list(currentPage){
      ...
      this.usersList = []
      let data = 'type=' + '0' + '&size=' + this.pageSize + '&current=' + this.currentPage + '&name=' + this.value2
        ...
        if(result.data.status == 0){
          if(result.data.data.records.length != 0){
            result.data.data.records.forEach(item => {
              ...
              this.usersList.push({id,name,tel,birthday,gender,grade,type,checked:false})
              this.usersList.forEach(item => {
