      ...
      <div class="query">
        ...
        <div class="btn">
          ...
          <el-button
            type="primary"
            icon="el-icon-plus"
            @click="$router.push('onePromotion_addActivity')"
          >添加</el-button>
        </div>
      </div>
      <div class="table">
        <el-table :data="activityList" style="width: 100%" height="598">
          ...
          <el-table-column prop="goodsA" label="商品A"></el-table-column>
          <el-table-column prop="goodsB" label="商品B"></el-table-column>
          ...
          <el-table-column label="操作" width="180">
            <template slot-scope="scope">
              ...
              <el-switch
                ...
                @change="update_stauts(scope.row.id)"
              ></el-switch>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    jump_detail(id) {
      ...
      this.$router.push("onePromotion_activityDetail?id=" + id);
    },
    jump_modify(id) {
      localStorage.setItem('path',JSON.stringify(this.$route.path))
      ...
      this.$router.push("onePromotion_activityModify?id=" + id);
    },
    get_list(currentPage) {
      ...
      let data =
        ...
        this.pageSize
      this.$axios
        .post("/web_one_promotion/promotion_list", data)
        .then(result => {
            ...
            if (data.length == 0) {
              ...
              data.forEach(item => {
                ...
                this.activityList.push({
                  ...
                  name: item.name,
                  price: item.price,
                  goodsA: item.goodsAName,
                  goodsB: item.goodsBName,
                  status,
    ...
    update_stauts(id) {
      this.$confirm('此操作将修改状态, 是否继续?', '提示', {
          ...
          this.$axios
        .post("/web_one_promotion/modify_promotion_status", "id=" + id)
        .then(result => {
          if (result.data.status == 0) {
