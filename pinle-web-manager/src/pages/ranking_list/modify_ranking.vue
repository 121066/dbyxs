<template>
  <div class="ranking_modify-container">
    ...
    <div class="content">
      <el-main>
        <el-collapse v-model="activeName">
          <el-collapse-item title="排行榜设置名称" name="1">
            <el-form ref="form" :model="form" label-width="80px">
              ...
              <el-form-item label="活动状态">
                <el-radio v-model="form.radio" label="0">禁用</el-radio>
                <el-radio v-model="form.radio" label="1" @change="change_status">启用</el-radio>
              </el-form-item>
            </el-form>
          </el-collapse-item>
          <el-collapse-item title="排行榜奖品设置" name="2">
            <div class="add_area" v-for="(item, index) in form.activitySetting" :key="index">
              ...
              <div class="btn">
                <div class="btn1">
                  <el-button @click="add_area(1,index)" v-show="item.isshow3">在其前添加区间</el-button>
                  <el-button @click="add_area(2,index)" v-show="item.isshow4">在其后添加区间</el-button>
                </div>
                <div class="btn2">
                  <el-button v-show="item.isshow1" @click="merge_area(1,index)">合并其前区间</el-button>
                  <el-button v-show="item.isshow2" @click="merge_area(2,index)">合并其后区间</el-button>
                </div>
              </div>
            </div>
          </el-collapse-item>
        ...
        <el-footer>
          <el-button type="primary" @click="send_ranking_modify">确 定</el-button>
...
export default {
  ...
  methods: {
    ...
    add_activity() {
      ...
      if (this.form.activitySetting[0].end == 1) {
        this.form.activitySetting[0].isshow3 = false;
        this.form.activitySetting[0].isshow4 = false;
      }
      if (
        this.form.activitySetting[this.form.activitySetting.length - 1].start ==
        100
      ) {
    ...
    send_ranking_modify() {
      ...
      rankingDetailVo["id"] = this.form.id;
      rankingDetailVo["status"] = this.form.radio;
      // formData.append("name", this.form.name);
      // formData.append('id',this.form.id)
      // formData.append('stauts',this.form.radio)
      let goods = [];
      this.form.activitySetting.forEach(item => {
      ...
      modify_ranking("rankingDetailVo=" + JSON.stringify(rankingDetailVo)).then(res => {
        if (res.data.status == 0) {
          if (res.data.data == 1) {
            return this.$message({
              type: "success",
      ...
      see_modify_detail("id=" + this.form.id).then(res => {
        if (res.data.status == 0) {
          ...
          goods.forEach((item, index) => {
            this.form.activitySetting.push({
              start: item.startRanking,
              end: item.endRanking,
              goods: {
                id: item.id,
