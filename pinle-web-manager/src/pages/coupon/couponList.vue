<template>
  <div class="coupon_list-container">
    ...
    <div class="content">
      <div class="query">
        ...
        <div class="btn">
          ...
          <el-button type="primary" icon="el-icon-plus" @click="$router.push('/addCoupon')">添加</el-button>
        </div>
      </div>
      <div class="table">
        <el-table :data="couponList" style="width: 100%" height="598">
          <el-table-column prop="name" label="优惠券名称"></el-table-column>
          <el-table-column prop="price" label="优惠券价值(元)"></el-table-column>
...
import { coupon_list, coupon_delete } from "../../api/getData.js";
export default {
  ...
  methods: {
    ...
    jump_detail(id) {
      ...
      localStorage.setItem("pageMsg", JSON.stringify({value1:this.value1,value2:this.value2,value3:this.value3,value4:this.value4,value5:this.value5,currentPage:this.currentPage,pageSize:this.pageSize}));
      this.$router.push("/coupon_detail?id=" + id);
    },
    jump_modify(id) {
      localStorage.setItem('path',JSON.stringify(this.$route.path))

  localStorage.setItem("pageMsg", JSON.stringify({value1:this.value1,value2:this.value2,value3:this.value3,value4:this.value4,value5:this.value5,currentPage:this.currentPage,pageSize:this.pageSize}));
      this.$router.push("/coupon_modify?id=" + id);
    },
    ...
    handleDelete(id) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          let data = "id=" + id;
          coupon_delete(data).then(result => {
            if (result.data.status == 0) {
    ...
    timeCompare(flag){
      ...
      if((new Date(this.value4)).getTime() >= (new Date(this.value5)).getTime()){
        ...
        return this.$message({
          type: 'warning',
          message: "'优惠券创建时间-开始'必须小于'优惠券创建时间-结束'"
        })
      }
    }
  }
