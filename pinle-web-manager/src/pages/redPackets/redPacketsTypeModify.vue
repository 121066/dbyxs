<template>
  <div class="redpac_type_modify-container">
    <breadcrumb style></breadcrumb>
    <div class="content">
      <el-main>
        <el-collapse v-model="activeName">
          <el-collapse-item title="红包类型名称" name="1">
          ...
          <el-collapse-item title="试用场景" name="2">
              ...
              <div class="msg_one" v-for="(item, index) in checkList4" :key="index">
                ...
                <div class="goodsType">
                  <div v-for="(item1, index1) in item.typeList" :key="index1">
                    <el-checkbox v-model="item1.checked" @change="isSelected(item.id)">{{item1.label}}</el-checkbox>
                  </div>
                </div>
              </div>
            </div>
            <div class="msg" v-if="radio == 5">
              <div class="msg_one" v-for="(item, index) in checkList5" :key="index">
                ...
                <div class="goodsType">
                  <div v-for="(item1, index1) in item.typeList" :key="index1">
                    <el-checkbox v-model="item1.checked" @change="isSelected_five(item.id)">{{item1.label}}</el-checkbox>&nbsp;
          ...
          <el-collapse-item title="红包类型备注" name="3">
        ...
        <el-footer>
          <el-button type="primary" @click="redpac_modify">确 定</el-button>
...
import { red_packet_type_detail ,red_packet_type_modify } from "../../api/getData.js";
export default {
  ...
  methods: {
    ...
    select_goods(goodsType, activityTyep,ids) {
      ...
      localStorage.setItem("msg", JSON.stringify({ goodsType, activityTyep,ids }));
    ...
    redpac_modify() {
      ...
      if (this.radio == 1) {
        redPacketTypeDetailVo = redPacketTypeDetailVo;
        ...
        redPacketTypeDetailVo.promotions = promotions;
        ...
        redPacketTypeDetailVo.gooodsTypes = gooodsTypes;
      } else if (this.radio == 4) {
        let promotionList = [];
        this.checkList4.forEach(item => {
          let redPacketTypePromotionVo = [];
          let goodsTypeList = [];
          if (item.checked == true) {
            redPacketTypePromotionVo = {
              id: item.id,
              promotionName: item.label
            };
            item.typeList.forEach(item1 => {
              if (item1.checked == true) {
                goodsTypeList.push({ id: item1.id, name: item1.label });
              }
            });
            promotionList.push({ redPacketTypePromotionVo, goodsTypeList });
          }
        });
        redPacketTypeDetailVo.promotionList = promotionList;
        ...
        redPacketTypeDetailVo.goodsIds = goodsIds;
      }
      let data =
        "redPacketTypeDetailVo=" + JSON.stringify(redPacketTypeDetailVo);
      red_packet_type_modify(data).then(result => {
        if (result.data.status == 0) {
          if (result.data.data == 1) {
            this.$router.push('/redPacketsType')
            this.$message({
              type:'success',
              message:'修改成功'
            });
