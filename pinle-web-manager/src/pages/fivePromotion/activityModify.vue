<template>
  <div class="fivemodifyact-container">
        ...
        <el-collapse v-model="activeName">
          ...
          <el-collapse-item title="拼团商品定制" name="2">
            <el-form label-width="120px" class="goods_select">
              <el-form-item :label="item.label" v-for="(item, index) in goods" :key="index">
                ...
                <span @click="select_goods(item.flag)">点击选择</span>
              </el-form-item>
            </el-form>
          </el-collapse-item>
          <el-collapse-item title="拼团价格/助力定制" name="3">
            <el-form ref="form" :model="form" label-width="135px" class="assemble">
              ...
              <el-form-item label="第二轮拼团价钱">
                <el-input v-model="form.moneyOne"></el-input>
                <span>元</span>
              </el-form-item>
              <el-form-item label="第三轮拼团价钱">
...
export default {
  ...
  methods: {
    ...
    get_detail(id) {
      this.$axios
        .post("/web_five_promotion/get_promotion", "id=" + id)
        .then(result => {
          if (result.data.status == 0) {
            let data = result.data.data;
            this.form = data;

            data.goodsDetail.forEach((item, index) => {
              if (data.goodsA == item.id) {
                this.goods[0]["msg"] = item;
              }
              if (data.goodsB == item.id) {
                this.goods[1]["msg"] = item;
              }
              if (data.goodsTwo == item.id) {
                this.goods[4]["msg"] = item;
              }
              if (data.goodsThree == item.id) {
                this.goods[2]["msg"] = item;
              }
              if (data.goodsFriend == item.id) {
                this.goods[3]["msg"] = item;
              }
            });
            if (data.fivePromotionShare.length != 0) {
              data.fivePromotionShare.forEach(item => {
                let goodsId = item.goodsId;
                let goods = [];
                data.shareGoodsDetail.forEach(item1 => {
                  if (goodsId == item1.id) {
                    goods = item1;
                  }
                });
            ...
            if (data.fiveReward.length != 0) {
              data.fiveReward.forEach((item, index) => {
                data.fiveRewardDetail.forEach(item1 => {
    ...
    update_activity() {
      ...
      this.$axios
        .post(
          "/web_five_promotion/modify_promotion",
